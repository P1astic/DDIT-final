...
<div id="container">
   <div id="title-box" style="display: flex; flex-direction: row; gap: 8px; margin: 10px; justify-content: space-between; align-items: center;">
      ...
      <div class="title-txt" style="color: rgba(0, 0, 0, 0.88);">발주현황</div>
      </div>
      <div>
...
<!-- 		<div class="sel-option-box" style="margin-left: 412px; width: 550px;"> -->
<!-- 	      <label for="itemM" style="width: 180px; margin-top: 7px;">중분류</label> -->
<!-- 	      <select id="itemM" name="itemM" class="form-control" style="width: 200px;"> -->
...
<!-- 	      <label for="itemS" style="width: 180px; margin-left: 10px; margin-top: 7px;">소분류</label> -->
<!-- 	      <select id="itemS" name="itemS" class="form-control" style="width: 400px;"> -->
...
<%--           <c:forEach var="itemVO" items="${articlePage.content}" varStatus="stat"> --%>
          <c:forEach var="itemOrderVO" items="${ItemOrderVOList}" varStatus="stat">
            <tr onclick="location.href='/emp/inventory/itemOrderDetail?orderNo=${itemOrderVO.orderNo}'">
               <td>${stat.index+1}</td>
               <td>${itemOrderVO.itemVO.itemNm}</td>
               <td>${itemOrderVO.orderQty}</td>
               <td><fmt:formatNumber pattern="#,###">${itemOrderVO.orderAmt}</fmt:formatNumber><span>원</span></td>
               <td>${itemOrderVO.employeeVO.empNm}</td>
               <td>${itemOrderVO.orderRegYmd}</td>
               <td>
               	   <c:choose>
               	   	 <c:when test="${itemOrderVO.orderStatus == 'Y'}"><div style="color: #007bff;">입고완료</div></c:when>
               		 <c:when test="${itemOrderVO.orderStatus == 'N'}"><div style="color: red;">미입고</div></c:when>
               	   </c:choose> 
               </td>
