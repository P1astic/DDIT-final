...
$(function() {	
	...
	$("#confirmSubmitBtn").on("click", function() {
		...
		$.ajax({
			...
			success : function(result) {
				...
				setTimeout(() => window.location.href="/bp/board/list", 1000);
			},
    ...
    $(".fileList").on("click", "#fileDown", function() {
        ...
        if (filePath) {
            window.location.href = "/bp/board/download?filePath=" + encodeURIComponent(filePath) + "&fileOrgnlFileNm=" + encodeURIComponent(fileOrgnlFileNm);
        } else {
                    	...
                    	<c:if test="${fn:length(memVO.authLinkVOList) > 0}">
	                        ...
	                        <c:forEach var="authLink" items="${memVO.authLinkVOList}">
		                        <c:if test="${(authLink.authCode == 'ROLE_ADMIN' || authLink.authCode == 'ROLE_FA_MNG') && !hasAccess}">
					                   	<a href="/bp/board/edit?ntcBrdNo=${boardVO.ntcBrdNo}" id="editBtn">수정</a>					                    
