...
<link rel="stylesheet" type="text/css" href="/resources/css/airplane/arrivalList.css">
...
let selDate;					// 선택한 날짜
let flightArray;				// 비행 스케쥴 배열
let matchCount;					// 선택한 날짜의 운항 개수
...
$(function(){
	...
	if(datefromMain == ''){
		$.ajax({
			url:"http://api.odcloud.kr/api/FlightStatusListDTL/v1/getFlightStatusListDetail?page="+page+"&perPage="+matchCount+"&cond%5BFLIGHT_DATE%3A%3AEQ%5D="+todayDateforLink+"&cond%5BIO%3A%3AEQ%5D=O&cond%5BAIRPORT%3A%3AEQ%5D=GMP&serviceKey="+serviceKey,
			type:"get",
		...
		console.log("airLineVO : " + "${airLineVO[1].airKrNm}");		
	}else{
	...
	$("#searchBtn").on("click", function() {
	 ...
	 // 날짜만 선택했을 때
	    if (date && (!arrived || arrived === "all") && (!airLine || airLine === "all") && (!airFln || airFln === "") && (!remark || remark === "all") && (!startTime || startTime === "0") && (!endTime || endTime === "2350")) {
	    	console.log("updateDate 호출");
	        updateDate();
	        
	    } else {
	    	console.log("날짜만 선택했을 때")
	    	$.ajax({
				url:"http://api.odcloud.kr/api/FlightStatusListDTL/v1/getFlightStatusListDetail?page="+page+"&perPage="+matchCount+"&cond%5BFLIGHT_DATE%3A%3AEQ%5D="+selDate+"&cond%5BIO%3A%3AEQ%5D=O&cond%5BAIRPORT%3A%3AEQ%5D=GMP&serviceKey="+serviceKey,
				type:"get",
				success:function(result){
					flightArray = result.data;
					console.log("날짜만 선택했을 때 데이터 개수 : ", flightArray.length)
					console.log("날짜만 선택했을 때 : ", flightArray);
					...
					flightArray.forEach((item, i) => {
		                ...
		                // 필터링 조건
		                if (arrived && arrived !== "all" && item.ARRIVED_KOR !== arrived) {
		                    match = false;
		                	console.log("arrived 부정");
		                }
	...
	// 김포공항처럼 시간 셀렉트 박스 생성하는 방법
	// 0000부터 2350까지 10분 단위로 값 생성
	for (let i = 0; i <= 2350; i += 10) {
	    ...
	    // 값을 네 자리로 맞추기 (예: 0000, 0010, 0020, ...)
	    let formattedTime = i.toString().padStart(4, '0');
	    
	    // ":"를 추가하여 형식 변경 (예: 00:00, 00:10, ...)
...
<!-- <select id="timeSelect" name="time"></select> -->
	...
	<div class="searchSection">
		...
		<div class="searchDiv">	
			<div class="searchChild">
				...
				<div class="searchSmall">
					<label for="arrived">목적지</label>
					<select id="arrived" name="arrived" class="form-control">
	...
	<div class="refreshDiv">
			<label id="lastRefresh">마지막 새로고침 : ${refreshDate}</label>		
