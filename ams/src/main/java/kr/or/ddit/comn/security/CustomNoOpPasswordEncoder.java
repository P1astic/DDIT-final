...
public class CustomNoOpPasswordEncoder implements PasswordEncoder {
	...
	public String encode(CharSequence rawPassword) {
		...
		//인코딩 프로세스가 없음. 인코딩을 안하겠다라는 의미se
		return rawPassword.toString();
	}

	@Override
	public boolean matches(CharSequence rawPassword, String encodedPassword) {
		log.warn("비밀번호 비교: 입력된 비밀번호(rawPassword) = " + rawPassword + ", 저장된 암호화된 비밀번호(encodedPassword) = " + encodedPassword);
		
////		// 입력된 비밀번호를 암호화
//	    String encodedPwd = this.passwordEncoder.encode(rawPassword);
...
//		return encodedPwd.equals(encodedPassword);
	}
