package kr.or.ddit.approv.util;

import java.util.List;
import java.util.Map;

//페이징 관련 정보 + 게시글 정보
public class waitPaymentPage<T> {
	//전체글 수
	private int total;
	//현재 페이지 번호
...
//	private int    amt;
	...
	//					753				1				10			select결과
	public waitPaymentPage(int total, int currentPage, int size, List<T> content
				, String keyword) {
...
//		this.gubun = map.get("gubun") != null ? (String) map.get("gubun") : "";
//		this.keyword = map.get("keyword") != null ? (String) map.get("keyword") : "";
...
//		this.amt = Integer.parseInt(map.get("amt").toString());
		...
		this.pagingArea += "href='/elecApv/waitPaymentList?gubun="+this.gubun+"&keyword="+this.keyword+"&currentPage="+(this.startPage-4)+"'>«</a></li>";
		for(int pNo=this.startPage;pNo<=this.endPage;pNo++) {
			...
			this.pagingArea += "'><a class='page-link' ";
			this.pagingArea += " href='/elecApv/waitPaymentList?gubun="+this.gubun+"&keyword="+this.keyword+"&currentPage="+pNo+"'>"+pNo+"</a></li>"; 
		}//end for
		this.pagingArea += "<li class='page-item";
		...
		this.pagingArea += "'><a class='page-link'";
		this.pagingArea += "href='/elecApv/waitPaymentList?gubun="+this.gubun+"&keyword="+this.keyword+"&currentPage="+(this.startPage+5)+"'>»</a></li>"; 
		this.pagingArea += "</ul>";	
	}//생성자 끝
