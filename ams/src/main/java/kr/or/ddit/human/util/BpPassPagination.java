...
public class BpPassPagination<T> {
	// 전체글 수
	private int total;
	// 현재 페이지 번호
	...
	//검색 생성자 시작
	public BpPassPagination(int total, int currentPage, int size, List<T> content, String keyword) {
	...
	//생성자
	public BpPassPagination(int total, int currentPage, int size, List<T> content, Map<String,Object> map) {
		//size : 한 화면에 보여질 목록의 행 수
				this.total = total;
				this.currentPage = currentPage;
				this.content = content;
				...
				this.pagingArea += "href='/bp/bpPass/requestList?startDate="+this.startDate+"&endDate="+this.endDate+
