package kr.or.ddit.auth.util;

import java.util.List;
import java.util.Map;

//페이징 관련 정보 + 게시글 정보
public class RolePagination<T> {
	//전체글 수
	private int total;
	//현재 페이지 번호
	...
	//					753				1				10			select결과
	public RolePagination(int total, int currentPage, int size, List<T> content
				, String keyword) {
	...
	public RolePagination(int total, int currentPage, int size, List<T> content
		...
		this.deptCd = (String) map.get("deptCd");
		...
		this.pagingArea += "href='/emp/role/list?authCode="+this.authCode+"&deptCd="+this.deptCd+"&currentPage="+(this.startPage-4)+"'>«</a></li>";
		for(int pNo=this.startPage;pNo<=this.endPage;pNo++) {
			...
			this.pagingArea += "'><a class='page-link' ";
			this.pagingArea += " href='/emp/role/list?authCode="+this.authCode+"&deptCd="+this.deptCd+"&currentPage="+pNo+"'>"+pNo+"</a></li>"; 
		}//end for
		this.pagingArea += "<li class='page-item";
		...
		this.pagingArea += "'><a class='page-link'";
		this.pagingArea += "href='/emp/role/list?authCode="+this.authCode+"&deptCd="+this.deptCd+"&currentPage="+(this.startPage+5)+"'>»</a></li>"; 
		this.pagingArea += "</ul>";	
	}//생성자 끝
