...
//페이징 관련 정보 + 게시글 정보
public class BoardPagination<T> {
	//전체글 수
	private int total;
	//현재 페이지 번호
	...
	//					753				1				10			select결과
	public BoardPagination(int total, int currentPage, int size, List<T> content
				, String keyword) {
	...
	public BoardPagination(int total, int currentPage, int size, List<T> content
		...
		this.deptGubun = (String) map.get("deptGubun");
		...
		this.pagingArea += "href='/emp/board/list?gubun="+this.gubun + "&deptGubun=" + this.deptGubun + "&keyword="+this.keyword + "&currentPage="+(this.startPage-4) +"'>«</a></li>";
		for(int pNo=this.startPage;pNo<=this.endPage;pNo++) {
			...
			this.pagingArea += "'><a class='page-link' ";
			this.pagingArea += "href='/emp/board/list?gubun="+this.gubun+ "&deptGubun=" + this.deptGubun + "&keyword="+this.keyword+"&currentPage="+pNo+"'>"+pNo+"</a></li>";
		}//end for
		this.pagingArea += "<li class='page-item";
		...
		this.pagingArea += "'><a class='page-link'";
		this.pagingArea += "href='/emp/board/list?gubun="+this.gubun + "&deptGubun=" + this.deptGubun + "&keyword="+this.keyword+"&currentPage="+(this.startPage+5)+"'>»</a></li>"; 
		this.pagingArea += "</ul>";	
	}//생성자 끝
